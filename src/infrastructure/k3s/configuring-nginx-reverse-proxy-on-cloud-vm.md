...
- **Action**: Create `/etc/nginx/sites-available/default` on `cloud-vm`:
  ...
  ```
- **Why**: Configures NGINX to forward all traffic for `example.com` and subdomains to the NGINX Ingress controller’s IP (`172.16.100.11`).
...
### 5. Update DNS
- **Action**: In your domain registrar’s DNS settings, set an `A` record for `example.com` and `*.example.com` to the `cloud-vm`’s public IP (e.g., `<CLOUD_VM_PUBLIC_IP>`).
