...
// Diagram: Colliding keys are stored consecutively in the internal array
...
// Diagram: Open Addressing

> Collision resolution schemes that resolve collisions by probing (searching) for alternate locations in the internal array are called open addressing schemes.

The linear probing implementation is quite simple. We calculate the hash value for a key to get its index and search the internal array linearly from that index, either of the key or an empty slot. We will look at the components that make up the hash table implemented using linear probing.
...
When we examine the implementation of search, insert, and delete operations, we will learn more about why we need to store this metadata explicitly. A record in the linear probing implementation is a data structure that encapsulates key-value pairs and type information for a slot (index) in the internal array.

// Diagram: A record in the linear probing implementation of a hash table
...
In the linear probing implementation of a hash table, the internal array stores all the data, so it is an array of records. Each slot in this array is either an empty or deleted record, as described by its `recordType` data member. We will learn later why it is important to distinguish between these two states.

// Diagram: The internal array is an array of records

To implement the internal array, we use the built-in array type or dynamic arrays from the standard library where we can. This is done to keep the implementation simple.
...
// Diagram: Representation of linear probing implementation of a hash table encapsulated in a class
...
Let's examine what happens when the code above is executed to understand better how encapsulating all the data and operations to implement a hash table using linear probing is useful.

// Diagram: Execution of code using an instance (object) of the hash table class

Now that we know what the linear probing implementation of a hash table looks like and how it functions, we will learn more about the implementation of each operation in the coming lessons.
...
// Diagram: Search for a key that is not present in the hash table and the table is not full
...
// Diagram: Search for a key that is not present in the hash table and the table is full
...
To implement the operation, we use the hash function to get the index in the internal array and then traverse the internal array from that index using linear probing to search for the given key. If the given key is found in the table, we return it's value. Otherwise, we return `-1` if we find an empty record, meaning we have searched through all the collisions but couldn't find the key.
...
// Diagram: export class MyHashTable {
    ...
    probeForOccupiedIndex(key: number, startIndex: number): number {
        for (let i = 0; i < this.capacity; ++i) {
            ...
            // Linear probing
            let probeIndex = (startIndex + i) % this.capacity;
...
export class MyHashTable {
    ...
    probeForOccupiedIndex(key, startIndex) {
        for (let i = 0; i < this.capacity; ++i) {
            ...
            // Linear probing
            const probeIndex = (startIndex + i) % this.capacity;
...
The search operation computes the hash value of the provided key, which is a constant-time operation. However, we must traverse the array from the calculated index to search for the key. In the best case, the record at the calculated index may have the given key, so the best-case time complexity is **constant**, **O(1)**.

In the worst case, however, all the keys stored in the hash table may have collided at the same hash value (index), so all occupied records would be stored consecutively. If the given key is not in the hash table, we must still traverse all these records to confirm this. And so, the worst-case time complexity of the search operation is **linear** **O(N)**.
...
// Diagram: Insert a key value mapping when the key is not present in the table
...
// Diagram: Insert a key value mapping when the key is not present and the table is full
...
We use the hash function to get the index in the internal array and implement the operation. We then combine all three cases using conditional statements and update the correct record in the array. Finally, we return a boolean value to indicate the operation's success.
...
Suppose the key is not found in any occupied record when searching the internal array using linear probing from the calculated index. In that case, the delete operation becomes a no-op (nothing is done). This can happen if we reach an empty record before finding the key.
...
In this case, the internal array is searched linearly, starting from the calculated index. Still, neither an unoccupied record nor a record with the given key is found in the entire traversal. This means that the internal array is full. In this case, the delete operation becomes a no-op (nothing is done).

// Diagram: Delete a key that is not present and the table is full

> **Algorithm**
>
> -   **Step 1:** Calculate the index(hash code) for the given key.
> -   **Step 2:** Traverse and search the array from that index using linear probing.
> -   **Step 3:** If the entire array is traversed without finding the key or an empty slot, terminate the operation (no-op).
...
In the worst case, however, all the keys stored in the hash table may have collided at the same hash value (index), so all occupied records would be stored consecutively. If the given key is not in the hash table, we must still traverse all these records to confirm this. And so, the worst-case time complexity of the delete operation is **linear** **O(N)**.
...
// Diagram: 2. Linear probing must be used as a collision resolution strategy
...
> **Explanation:**
>
> **Operation:** MyHashTable myHashTable = new MyHashTable(3) **Result:** Initializes an empty \`MyHashTable\` with a capacity of 3
>
